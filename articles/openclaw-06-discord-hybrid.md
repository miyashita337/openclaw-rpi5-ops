...
本記事で **Discord 受信窓口は RPi5 で 24h 常駐** までは完了しました。次回 #7 では、**LLM 推論を OpenAI API から Windows タワーの self-hosted LLM に切り替える** ところを書く予定です。執筆時点 (2026-05-17) でほぼ実装まで進んだ要素もあるので、現状の進捗もあわせて先出ししておきます。

- **Windows タワー self-hosted LLM の runtime 選定**: ✅ **Ollama + Qwen3.6:27b Q4** に確定 ([#14](https://github.com/miyashita337/openclaw-rpi5-ops/issues/14))。`/api/chat` (Ollama native) + `think:false` で reasoning を切らないと Qwen3.6 系は `/no_think` directive が効かず thinking が混入する罠を踏んだので、OpenAI 互換 `/v1` ではなく native API を採用しています
- **Windows タワーの常時公開**: ✅ schtasks `OllamaServe` で AT_LOGON 起動 + Tailscale ACL ([#17](https://github.com/miyashita337/openclaw-rpi5-ops/issues/17))。30 連続 GET 成功 + 5 prompt bench 完走 ([#17 AC1/AC2/AC4](https://github.com/miyashita337/openclaw-rpi5-ops/issues/17))
- **RPi5 → Windows LLM forward 経路**: ✅ **設計ピボット**。当初 OpenClaw 内蔵 agent から forward させようとしましたが、session affinity / gateway token / mention filter の壁が多く、AgentTeams 診断で「要件外の agentic framework は overkill」と全員一致でピボット推奨されたため、**純粋な forward だけを担う最小 bot** (`scripts/discord-ollama-bridge.py`) を別 systemd unit (`discord-ollama-bridge.service`) で常駐させる方針に切り替えました ([#11 Phase 2](https://github.com/miyashita337/openclaw-rpi5-ops/issues/11))。OpenClaw 自体は OpenAI 経路として温存
- **bridge bot の hang 対策**: ✅ Qwen3.6 の冗長応答で 300s timeout に到達した事故を観測 → `options.num_predict` を 4096 → 512 に絞って回避 (commit 3868589)
- **Win Tower スリープ→復帰サイクル**: ⚠️ 物理操作を伴う実機 spike として [#18](https://github.com/miyashita337/openclaw-rpi5-ops/issues/18) に分離。schtasks `/SC ONLOGON` 起動なので「再ログオン要かどうか」が Win 11 24H2 の電源管理次第
- **Bot SOUL.md (境界線) 設定**: 「こういう振る舞いは好き / 苦手 / 勝手にやって OK / 必ず確認して」を Discord で会話設定 (持ち越し)
- **複数 channel 統合**: 現状 1 channel + 1 DM のみ → 複数 channel に Bot を配置して「用途別 routing」できるようにする (持ち越し)
- **Bot 応答速度の比較**: OpenAI 経路の `@mention → 👀 reaction → 返答` 体感 5-10 秒に対して、Ollama 経路の wall-clock も計測予定 (#14 単体測定で 12-19 秒、Discord 経路を通した end-to-end は次回計測)
