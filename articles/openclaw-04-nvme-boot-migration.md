...
[前回の記事](https://zenn.dev/harieshokunin/articles/9caa728de35b1f) で Raspberry Pi 5 (`hostname.local`) を初期セットアップした。
...
Tailscale 管理画面 (login.tailscale.com/admin/machines) にも `hostname` が **`SSH` バッジ付き** で現れる。
...
常時稼働の自宅サーバーが定期的に切れるのは事故の元なので、hostname に限り無効化しておく。

login.tailscale.com/admin/machines で hostname の `…` メニューから:
