---
title: "OpenClaw自動化サーバー構築記 #5 OpenClaw導入 — 公式 install.sh を監査して /opt 昇格 + system user + systemd hardening でスコア 9.0 → 1.4"
emoji: "🛡"
type: "tech"
...
2. install path は **C-1 方式 (git install を harieshokunin で完走 → `/opt/openclaw` に rsync 昇格 → system user `openclaw` で動作)** を採用。理由: `git diff` で更新が読める / commit hash で revision pin できる / 公式想定 (current user 直起動) より blast radius が小さい
...
![systemd-analyze security の Before/After 比較 (TODO: スクショ撮影)](/images/openclaw-05/01-score-before-after.png)

## はじめに

[前回 #4](https://zenn.dev/harieshokunin/articles/) で NVMe ブート移行を済ませた `wells` (Raspberry Pi 5) に、本連載の主役である **OpenClaw** を本番運用構成で入れる。

OpenClaw は AI アシスタントと自動化を繋ぐ daemon で、agent 機能 / channels / browser control / cron 等を一台に同居させる。 個人ホームサーバーで動かすため、以下を満たす構成を目指した:

- **harieshokunin (個人ユーザー) の権限を持たない** 専用 system user で動作 (compromise 時の被害範囲を限定)
...
| **B** | 公式 git install (harieshokunin の HOME に直接) | コードは読める | 個人ユーザー直起動のため compromise 時の被害大 |
...
sudo rsync -aH --delete \
  ...
  /home/harieshokunin/openclaw/ /opt/openclaw/
...
:::message alert
**ここで 1 回ハマった**: 当初は heredoc (`sudo tee /etc/openclaw/openclaw.env <<EOF ... EOF`) で書こうとしたが、SSH 越しに `iTerm2 → wells` の paste を経由するとインデントが混入し、終端 `EOF` が認識されず bash が `>` 継続プロンプトで永久 hang した。 **解決策は heredoc を使わず `printf '%s\n' 'line1' 'line2' | sudo tee /path > /dev/null` の単一行パターンに統一**。 RW (rework pattern) として登録済。
:::
...
![/tmp ro 衝突 → PrivateTmp で解消の対比 (TODO)](/images/openclaw-05/04-protect-system-strict-issue.png)
...
![systemd-analyze security の最終出力 (TODO)](/images/openclaw-05/05-final-score.png)
...
1. **Mac → wells SSH 許可**: プロジェクトの `.claude/settings.json` の `allow` に `Bash(ssh harieshokunin@wells:*)` を追加 (deny の `Bash(ssh:*)` は project scope で削除)
2. **wells 側 NOPASSWD**: `/etc/sudoers.d/openclaw-claude-ops` を作って systemd 操作のみ NOPASSWD に。 撤去は `sudo rm` 1 回でゼロに戻る

```bash
# 付与 (作業中のみ)
cat > /tmp/sudoers <<'EOF'
harieshokunin ALL=(root) NOPASSWD: /bin/systemctl daemon-reload
harieshokunin ALL=(root) NOPASSWD: /bin/systemctl restart openclaw-gateway
... (operations 限定で 15 行)
EOF
...
これで Claude が `ssh harieshokunin@wells "sudo -n systemctl restart openclaw-gateway"` のような sudo 込みコマンドを **passwordless** で打てる。 hardening 作業 30 分のうち、ユーザー操作は **NOPASSWD 付与の 1 回のみ** に圧縮された。
...
| 3 | `du -sh /opt/openclaw` が `4.0K` と表示される | `chmod -R u=rwX,g=rX,o=` で o= したため harieshokunin が中身を読めない | `sudo du` で打ち直すと正しく 2.9G。 「rsync が空コピー失敗した」ではなく権限で見えなかっただけ |
...
- **Web UI build**: `Control UI build failed: Missing UI runner: install pnpm` → harieshokunin で build → `/opt/openclaw/dist/web` 等を rsync で同梱する設計 (build chain を runtime user に持たせない方が secure)
- **port forward 動作確認**: Mac から `ssh -N -L 18789:127.0.0.1:18789 harieshokunin@wells` → `http://localhost:18789/` で token 認証
